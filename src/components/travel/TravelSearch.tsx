...
export const TravelSearch: React.FC<TravelSearchProps> = ({
  ...
  const [suggestions, setSuggestions] = useState<{ origin: SuggestionItem[]; destination: SuggestionItem[] }>({
    origin: [],
    destination: []
  });
  ...
  const isRoundTrip = activeService === 'flights' && params.tripType === 'round-trip';

  const canSubmit = useMemo(() => {
    if (!params.origin || !params.destination || !params.departureDate) return false;
    if (isRoundTrip && !params.returnDate) return false;
    return true;
  }, [params, isRoundTrip]);
  ...
  const handleTripTypeChange = (tripType: 'round-trip' | 'one-way') => {
    setParams((prev) => ({
      ...prev,
      tripType,
      returnDate: tripType === 'one-way' ? '' : prev.returnDate || format(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), 'yyyy-MM-dd')
    }));
    setErrorMessage('');
  };

  const handleSelectSuggestion = (field: 'origin' | 'destination', city: SuggestionItem) => {
    if (field === 'origin') {
      setOriginInput(`${city.name} (${city.code})`);
      setParams((prev) => ({ ...prev, origin: city.code }));
    } else {
      setDestinationInput(`${city.name} (${city.code})`);
      setParams((prev) => ({ ...prev, destination: city.code }));
    }
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    if (!params.origin || !params.destination) {
      setErrorMessage('Choisis un départ et une destination.');
      return;
    }

    if (!params.departureDate) {
      setErrorMessage('Choisis une date de départ.');
      return;
    }

    if (isRoundTrip && !params.returnDate) {
      setErrorMessage('Choisis une date de retour.');
      return;
    }
    ...
    <div className="w-full max-w-5xl mx-auto -mt-8 md:-mt-16 relative z-10 px-4">
      <div className="bg-white shadow-2xl rounded-2xl p-4 md:p-6 border border-zinc-100">
        ...
        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setParams((prev) => ({ ...prev, destination: '' }));
          ...
          <div className={`${isRoundTrip ? 'md:col-span-2' : 'md:col-span-4'} space-y-1`}>
            <label className="text-[10px] uppercase tracking-widest font-bold text-zinc-400 ml-1">
              Départ
            </label>
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => {
                  setParams((prev) => ({ ...prev, departureDate: e.target.value }));
                  setErrorMessage('');
                }}
                className="w-full pl-10 pr-4 py-3 bg-zinc-50 border-2 border-transparent focus:border-black rounded-xl text-sm text-zinc-900 transition-all outline-none"
              />
            ...
            <div className="md:col-span-2 space-y-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => {
                    setParams((prev) => ({ ...prev, returnDate: e.target.value }));
                    setErrorMessage('');
                  }}
                  className="w-full pl-10 pr-4 py-3 bg-zinc-50 border-2 border-transparent focus:border-black rounded-xl text-sm text-zinc-900 transition-all outline-none"
                />
          ...
          <div className="md:col-span-2 flex items-end">
            <button
              ...
              className={`w-full py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-xl active:scale-[0.98] ${
          ...
          {activeService === 'flights' && (
            <>
              <select
                ...
                onChange={(e) => setParams((prev) => ({ ...prev, cabinClass: e.target.value as any }))}
              ...
              <button
                type="button"
                onClick={() =>
                  setParams((prev) => ({ ...prev, directFlights: !prev.directFlights }))
                }
              ...
              >
                ...
                <span className={params.directFlights ? 'text-black font-medium' : ''}>
